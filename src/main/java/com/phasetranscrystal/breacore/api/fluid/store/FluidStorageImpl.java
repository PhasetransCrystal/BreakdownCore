        ...
        toRegister.entrySet().stream()
                ...
                .forEach(entry -> {
                    if (map.containsKey(entry.getKey())) {
                        BreaCore.LOGGER.warn("{} already has an associated fluid for material {}", entry.getKey(),
                                material);
                        return;
                    }
                    ...
                    if (!storeNoOverwrites(entry.getKey(), fluid, entry.getValue())) {
                        BreaCore.LOGGER.error("{} already has an associated fluid for material {}", material, material);
                    }
                });
